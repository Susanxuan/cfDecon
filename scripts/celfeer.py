...
def simulate_arrays(sample, num_samples, num_unk, proportions=None):
    ...
    tissue_totaldepths = np.sum(ref_2, axis=(1, 2))
    x_2 = (ref_2.T / tissue_totaldepths).T * np.average(tissue_totaldepths)
...
def plot_scatter_plots(beta_est, beta_truth, input_path, file):
    ...
    fig.savefig('./results/figure/' + "celfeer" + input_path + file + '0.5.png')
...
if __name__ == "__main__":
    startTime = time.time()
    for input_path in ["./data/UXM_ref25_100m.bed"]:
        ...
        output_alpha_file = f"{output_directory}/{parallel_job_id}_tissue_proportions.txt"
        output_beta_file = f"{output_directory}/{parallel_job_id}_methylation_proportions.txt"
        ...
        for file in ["0.3False0.4", "0.5False0.4"]:  # "0.1False0.4", "0.3False0.4", "0.5False0.4",
            if train == 1:
                print(file)
                sparse, ifrare, rare = extract_values(file)
                proportions = "./results/fractions/sample_array_100025_spar" + file + ".pkl"  #

                props = pkl.load(open(proportions, 'rb'))
                x, y, true_beta = simulate_arrays(data_df, int(num_samples), int(unknowns), proportions=props)
                data = torch.tensor(x)
                ...
                print('l1, ccc:', l1, ccc)
                with open(output_directory + "/celfeer" + input_path + file + "alpha_est.pkl", "wb") as f:
                    pkl.dump(alpha_max, f)
                with open(output_directory + "/celfeer" + input_path + file + "alpha_true.pkl", "wb") as f:
                    pkl.dump(props, f)
                with open(output_directory + "/celfeer" + input_path + file + "beta_est.pkl", "wb") as f:
                    pkl.dump(beta_max, f)
        ...
        if test == 1:
            ...
            with open(output_directory + "/test" + "celfeer" + input_path + "alpha_est.pkl", "wb") as f:
                pkl.dump(alpha_max, f)
            with open(output_directory + "/test" + "celfeer" + input_path + "beta_est.pkl", "wb") as f:
                pkl.dump(beta_max, f)
            with open(output_directory + "/test" + "celfeer" + input_path + "beta_true.pkl", "wb") as f:
    ...
    # write_output(output_alpha_file, alpha_max, tissues, samples, unknowns)
    # write_output(
    #     output_beta_file, np.transpose(beta_max, (1, 0, 2)), tissues, list(range(len(beta_max[1]))), unknowns
    # )
