...
def plot_scatter_plots(beta_est, beta_truth, input_path, file):
    ...
    fig.savefig('./results/figure/' + "celfeer" + input_path + "sim1" + '.png')
...
def simulate_arrays(sample, num_samples, num_unk, proportions=None):
    ...
    x_2 = np.array(np.split(reference_1, num_tissues, axis=1))
    tissue_totaldepths = np.sum(x_2, axis=(1, 2))
    x_2 = (x_2.T / tissue_totaldepths).T * np.average(tissue_totaldepths)
    x_2_percents = x_2.reshape(x_2.shape[0], x_2.shape[1] * x_2.shape[2])
    mix_x_2_percents = np.dot(proportions, x_2_percents)
...
if __name__ == "__main__":
    ...
    # prop只和train有关
    for input_path in ["./data/WGBS_sim_input.txt"]: #"WGBS_ref_2+1.txt",
        for file in ["0.1False0.4", "0.3False0.4", "0.5False0.4", "0.3True0.1", "0.3True0.2", "0.3True0.3"]:
            ...
            if train == 1:
                data_df = pd.read_csv(
                    input_path, delimiter="\t", header=None, skiprows=1)
