...
def generate_sample_array(n, c, sparse=False, sparse_prob=0.3, rare=False, rare_percentage=0.4, WBC_major=False, UXM=False, fix_c=False):
    if WBC_major:
        print(
        ...
        for i in range(n):
            if fix_c:
                ...
                indices = np.random.choice(np.arange(other_prop.shape[1]), replace=False, size=random.randint(c-5-9, c-5-1))
            print(indices)    
...
sample_array = generate_sample_array(n, c, UXM=True, fix_c=False)
filename = "/data2/yixuan/cfDNA_Decon/CelFEER/new_data_output/fractions/UXM_unfix_c_10.pkl"
