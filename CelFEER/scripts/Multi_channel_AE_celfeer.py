...
from train import reproducibility, test_AE_function
...
def simulate_arrays(sample, num_samples, num_unk, proportions=None, simi='2'):
    """
    takes input data matrix- cfDNA and reference, and creates the arrays to run in EM. Adds
    specified number of unknowns to estimate
...
if __name__ == "__main__":
    ...
    if args.proportions:
        ...
        if args.simi=='2':
            # simulated 2 data
            simulated_x_1, true_beta_1, simulated_x_2, true_beta_2 = simulate_arrays(data_df, int(args.num_samples), int(args.unknowns),proportions=props, simi=args.simi)
            ...
            x, true_beta = simulate_arrays(data_df, int(args.num_samples), int(args.unknowns),proportions=props, simi=args.simi)
            data=torch.tensor(x)
        ...
        CCC = []
        if args.adaptive:
            l1, ccc, pred, sign = test_AE_function(train_x,train_y,test_x,test_y,outdir=args.output_directory,adaptive=args.adaptive, mode=args.mode)
            print('l1, ccc:',l1, ccc)
            L1.append(l1)
            CCC.append(ccc)
        ...
        # make input arrays and add the specified number of unknowns
        test_x, reference = define_arrays(data_df, int(args.num_samples), int(args.unknowns))
    ...
    # write_output(output_alpha_file, props, tissues, samples, args.unknowns)
    # write_output(
    #     output_beta_file, np.transpose(sign, (1, 0, 2)), tissues, list(range(len(sign[1]))), args.unknowns
    # )
