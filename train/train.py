...
from model import simdatset, cfDecon, baseline, device
...
def adaptive_stage(model, data, optimizerD, optimizerE, step=10, max_iter=5, alpha=20, beta=1, true_beta=None):
    ...
    for k in range(max_iter):
        ...
        for i in range(step):
            ...
            # batch_loss = F.l1_loss(x_recon, data) + beta * F.l1_loss(sigm, ori_sigm)
            if true_beta is None:
                batch_loss = F.l1_loss(x_recon, data) + beta * F.l1_loss(sigm, ori_sigm)
            else:
                batch_loss = F.l1_loss(x_recon, data) + beta * F.l1_loss(sigm, true_beta)  # 10
            batch_loss.backward()
            optimizerD.step()
...
def predict(test_x, genename, celltypes, samplename,
            ...
            adaptive=True, mode='high-resolution', outdir=None, alpha=20, beta=1, true_beta=None):
...
def test_AE_function(train_x, train_y, test_x, test_y, batch_size=128, outdir=None, adaptive=True, mode='overall',
                     r1=None, r2=None ,alpha=20, beta=1, use_norm=False, true_beta=None):
    reproducibility(seed=0)
    train_loader = DataLoader(simdatset(train_x, train_y), batch_size=batch_size, shuffle=True)
    model = cfDecon(train_x.shape[1], train_x.shape[2], train_y.shape[1], r1, r2, use_norm).to(device)
    #model = baseline(train_x.shape[1], train_x.shape[2], train_y.shape[1]).to(device) # Baseline method
    ...
    print("Number of params of occupied memory:%.3fMb" % (num_params * 4 / 1024 / 1024), '\n')
    ...
    if adaptive:
        pred, train_sigm = predict(test_x=test_x, genename=genename, celltypes=celltypes, samplename=samplename,
                                   model=model, adaptive=adaptive, mode=mode, outdir=outdir, alpha=alpha, beta=beta, true_beta=true_beta)
...
def predict_AE_function(train_x, train_y, test_x, num_cells, batch_size=128, outdir=None, adaptive=True,
                        mode='overall', r1=None, r2=None, alpha=20, beta=1, use_norm=False, true_beta=None):
    reproducibility(seed=0)
    train_loader = DataLoader(simdatset(train_x, train_y), batch_size=batch_size, shuffle=True)
    model = cfDecon(train_x.shape[1], train_x.shape[2], train_y.shape[1], r1, r2, use_norm).to(device)
    #model = baseline(train_x.shape[1], train_x.shape[2], train_y.shape[1]).to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-3)  # 1e-4
    p = [x for x in model.parameters()]
    num_params = sum([param.numel() for param in p])
    ...
    showloss(reconloss, outdir + 'pred_loss_re.png')
    genename = range(test_x.shape[1])
