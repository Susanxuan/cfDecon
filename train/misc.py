...
from train import reproducibility, test_AE_function, predict_AE_function
from utils import generate_sample_array, generate_sample_array
...
def simulate_arrays(sample, num_samples, num_cell, proportions=None, simi=1, sparse=None, ifrare=None, rare=None):
    """
    takes input data matrix- cfDNA and reference, and creates the arrays to run in EM. Adds
    specified number of unknowns to estimate
    ...
    reference_2 = sample.iloc[:, (num_cell) * 5 + 3 + 3:].values
...
def simulate_arrays_new(sample, num_samples, proportions=None, simi=1, n=None, fix_c=None, c=None, WBC_major=False,
                                                   UXM=False):
    """
    takes input data matrix- cfDNA and reference, and creates the arrays to run in EM. Adds
    specified number of unknowns to estimate
    ...
    if proportions is not None:
        ...
        training_props = generate_sample_array(n, c=c, sparse=False, sparse_prob=0.3, rare=False, rare_percentage=0.4,
                                                   WBC_major=WBC_major,
                                                   UXM=UXM, fix_c=(fix_c == str(True)))
        x_2 = np.array(np.split(reference_2, num_tissues, axis=1))
        tissue_totaldepths = np.sum(x_2, axis=(1, 2))
        x_2 = (x_2.T / tissue_totaldepths).T * np.average(tissue_totaldepths)
        x_2_percents = x_2.reshape(x_2.shape[0], x_2.shape[1] * x_2.shape[2])
...
def simulate_arrays_miss(sample, num_samples, num_cell, proportions=None, simi=1, miss=None):
    """
    takes input data matrix- cfDNA and reference, and creates the arrays to run in EM. Adds
    specified number of unknowns to estimate
    ...
    num_unk: number of unknowns to estimate
    """
    reference_1_ori = sample.iloc[:, 3: (num_samples) * 5 + 3].values

    reference_2_ori = sample.iloc[:, (num_cell) * 5 + 3 + 3:].values
    ...
    reference_1 = reference_1_ori[:, keep_indices]
    ...
    ref_1 = np.array(np.split(reference_1, num_tissues - miss, axis=1))
    if proportions is not None:
        if simi == 2:
            x_1 = np.array(np.split(reference_1_ori, num_tissues, axis=1))
            tissue_totaldepths = np.sum(x_1, axis=(1, 2))
            x_1 = (x_1.T / tissue_totaldepths).T * np.average(tissue_totaldepths)
            x_1_percents = x_1.reshape(x_1.shape[0], x_1.shape[1] * x_1.shape[2])
            mix_x_1_percents = np.dot(proportions[:int(proportions.shape[0] / 2), :], x_1_percents)
